 ...
 * @module Login
 *
 * Provides the optimized entry point for user authentication.
 * Handles redirection to the organization's SSO provider and
 * displays authentication-related errors or status messages.
 *
 * Implements tree-shakeable MUI imports and lazy-loaded feedback components
 * protected by an ErrorBoundary for optimal First Contentful Paint (FCP).
 */
...
// Lazy-load non-critical feedback / alert components
const LazyLoginFeedback = lazy(() => import("./LoginFeedback"));

/**
 * The primary login page component.
 *
 * Manages the SSO authentication flow, including fetching the SSO URL
 * from the backend and redirecting the user. Non-critical feedback UI
 * (toasts, error alerts) is dynamically loaded with Suspense and ErrorBoundary.
 *
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* Lazy-loaded Feedback & Toast Alerts wrapped in ErrorBoundary & Suspense */}
                {(toast.open || Boolean(urlError)) && (
                    <ErrorBoundary
                        fallback={
                            <Box sx={{ width: "100%", textAlign: "center", p: 1 }}>
                                <Typography color="error" variant="body2">
                                    {toast.message || "Authentication error occurred. Please try again."}
                                </Typography>
                            </Box>
                        }
                    >
