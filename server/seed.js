 ...
 * @module Seed
 *
 * Database seeding script for local development and initial environment bootstrap.
...
const NOC_SHIFT_TYPES = [
    { _id: shiftTypeMorningId, name: "בוקר", color: "#476db5", isVacation: false },
    { _id: shiftTypeEveningId, name: "ערב", color: "#a32e9d", isVacation: false },
    { _id: shiftTypeNightId, name: "לילה", color: "#2f3436", isVacation: false },
    { _id: shiftTypeAfterId, name: "אפטר", color: "#bac4c8", isVacation: false },
    { _id: shiftTypeMiddleId, name: "אמצע", color: "#2c728e", isVacation: false },
    { _id: shiftTypeWeekendId, name: "שבת", color: "#eee836", isVacation: false },
    { _id: shiftTypeVacationId, name: "חופש", color: "#E57373", isVacation: true },
    { _id: shiftTypeLeaveId, name: "חול", color: "#9d6262", isVacation: true },
];
...
const importData = async () => {
    try {
        ...
        await ShiftReport.create({
            groupId: gMap["noc"],
            title: `משמרת בוקר - ${reportStartTime.toLocaleDateString("he-IL").replace(/\./g, "/")}`,
            date: reportStartTime,
            startTime: reportStartTime.toISOString(),
            endTime: reportEndTime.toISOString(),
            previousTasks: "<p>המשך מעקב אחר שרתים ראשיים ובדיקת גיבויים שגרתית.</p>",
            currentTasks: "<p><strong>סטטוס משמרת:</strong> הכל תקין, ללא תקלות חריגות.</p><ul><li>בוצעו בדיקות תקשורת מול כלל האתרים.</li><li>תחזוקת שרתים הסתיימה בהצלחה.</li></ul>",
            attendees: [
                {
                    userId: createdUsers[0]._id,
