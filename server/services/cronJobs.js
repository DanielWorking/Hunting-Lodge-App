            ...
            try {
                ...
                for (const slot of group.settings.timeSlots) {
                    ...
                    if (Math.abs(slotTimeVal - currentTimeVal) <= 1) {
                        ...
                        // Generate a unique title for the report based on slot name and Hebrew locale date in Asia/Jerusalem.
                ...
                console.error(`[Cron] Error processing group ${group?.name || group?._id}:`, groupError);
            }
        }
    } catch (error) {
